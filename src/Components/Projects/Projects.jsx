...
const PipelineVisual = ({ project, isActive }) => {
    ...
    return (
      <div className="flex flex-col items-center justify-center gap-3 py-8">
        ...
        <div className="mt-4 flex items-center gap-2 text-xs font-mono text-white/40">
          <Play size={12} className="text-accent-light" /><span>Hover to run pipeline</span>
        </div>
      </div>
    );
  }
  if (project.visual === "architecture" && project.pipeline.length > 0) {
    return (
      <div className="flex flex-col items-center justify-center gap-3 py-8">
        {project.pipeline.map((stage, i) => (
          <div key={stage} className="flex flex-col items-center gap-3">
            <motion.div initial={{ opacity: 0.3 }} whileInView={{ opacity: 1 }} transition={{ delay: i * 0.1 }} className="px-6 py-4 rounded-xl border border-accent/20 bg-base-800/60 backdrop-blur-sm flex items-center gap-3 min-w-[180px]">
  ...
  return (
    <div className="rounded-xl border border-white/10 bg-base-800/40 overflow-hidden">
      <div className="flex items-center gap-2 px-4 py-3 border-b border-white/8 bg-base-900/60">
        <div className="w-2.5 h-2.5 rounded-full bg-red-500/60" />
        <div className="w-2.5 h-2.5 rounded-full bg-yellow-500/60" />
        <div className="w-2.5 h-2.5 rounded-full bg-green-500/60" />
        <div className="ml-3 flex-1 h-5 rounded bg-base-700/50" />
      </div>
      <div className="h-48 md:h-56 flex items-center justify-center bg-gradient-to-br from-base-800/30 to-base-900/50">
        <div className="text-center">
          <Box size={40} className="text-white/15 mx-auto mb-2" />
          <span className="text-sm font-mono text-white/30">{project.title}</span>
        </div>
      </div>
    </div>
  );
  ...
  return (
    ...
    >
      <div className={isReversed ? "lg:order-2" : ""}>
        <div data-cursor="VIEW" className="group relative rounded-2xl border border-white/10 bg-base-900/40 overflow-hidden hover:border-accent/20 transition-all">
          <div className="bg-grid-fine absolute inset-0 opacity-30" />
          <div className="relative p-6 md:p-8">
            <PipelineVisual project={project} isActive={true} />
          </div>
        </div>
      </div>
      <div className={isReversed ? "lg:order-1" : ""}>
        ...
        <p className="text-base text-white/55 leading-relaxed mb-6 max-w-lg">{project.description}</p>
        <div className="flex flex-wrap gap-2 mb-6">
          {project.tech.map((t) => (
            <span key={t} className="px-3 py-1 rounded-full border border-white/10 bg-base-800/40 text-xs font-mono text-white/50">{t}</span>
          ))}
        </div>
    ...
    <section id="Work" className="relative px-6 md:px-12 lg:px-16 py-20 md:py-32">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex flex-wrap gap-2 mb-12">
          {filters.map((f) => (
            <button key={f} onClick={() => setFilter(f)} data-cursor="" className={`px-4 py-2 rounded-full text-xs font-mono uppercase tracking-wider transition-all ${filter === f ? "bg-accent text-white border border-accent" : "text-white/40 border border-white/10 hover:border-white/20 hover:text-white/70"}`}>
              {f}
            </button>
          ))}
        </div>
