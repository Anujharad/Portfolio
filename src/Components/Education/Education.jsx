  ...
  <section className="relative px-6 md:px-12 lg:px-16 py-20 md:py-32">
    <div className="max-w-7xl mx-auto">
      <div className="flex items-center gap-3 mb-12">
        <span className="text-xs font-mono text-accent">05 / EDUCATION</span>
        ...
        <div>
          ...
          <motion.div {...stagger} className="space-y-4">
            {certifications.map((cert, i) => (
              <motion.div key={i} {...staggerItem} className="group flex items-center justify-between p-5 rounded-xl border border-white/5 bg-base-800/40 hover:border-accent/20 transition-all">
                <div className="flex items-center gap-4">
                  <div className="w-10 h-10 rounded-lg border border-white/10 bg-base-700/40 flex items-center justify-center">
                ...
                {cert.url ? (
                  <a href={cert.url} target="_blank" rel="noopener noreferrer" data-cursor="" className="flex items-center gap-1 text-xs font-mono text-gray-500 hover:text-accent-light transition-colors">
                    View <ArrowUpRight size={14} />
                  </a>
                ) : (
                  <span className="text-xs font-mono text-gray-700">Verified</span>
                )}
              </motion.div>
            ))}
          </motion.div>
